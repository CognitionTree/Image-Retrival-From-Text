...
#ploting distance vs prob
def bynary_split(points, labs):
	xs0 = []
	ys0 = []
...
def plot_2d(points, labs):
	xs0, ys0, xs1, ys1 = bynary_split(points, labs)

	fig = pl.figure()
	ax = fig.add_subplot(111)

	ax.set_xlabel('distance')
	ax.set_ylabel('Probability of going from first to second camera')

	ax.plot(xs0, ys0, 'rx', xs1, ys1, 'bx')
	pl.show()
	fig.savefig('distance_prob_test.png')
	pl.close(fig)	
