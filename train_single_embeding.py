...
def create_base_text_network_lstm(input_shape):
	kernel_size = (5,5)
	embedding_size = 128
	model = Sequential()

	model.add(Dense(embedding_size, activation='relu', kernel_regularizer=regularizers.l2(0.005), input_shape=input_shape))
...
T = T.astype('float32')
numb_examples, word_emb = T.shape
...
base_text_network = create_base_text_network_lstm(input_shape_text)
plot_model(base_image_network, to_file='base_image_network_1dsentence.png', show_shapes=True, show_layer_names=True)
plot_model(base_text_network, to_file='base_text_network_1dsentence.png', show_shapes=True, show_layer_names=True)
...
plot_model(model, to_file='full_model_1dsentence.png', show_shapes=True, show_layer_names=True)
rms = RMSprop()
